...
main()
{
	...
	if(getcvar("scr_kc_timelimit") == "")		// Time limit per map
		setcvar("scr_kc_timelimit", "30");
	else if(getcvarfloat("scr_kc_timelimit") > 1440)
		setcvar("scr_kc_timelimit", "1440");
	level.timelimit = getcvarfloat("scr_kc_timelimit");

	if(getcvar("scr_kc_scorelimit") == "")		// Score limit per map
		setcvar("scr_kc_scorelimit", "50");
	level.scorelimit = getcvarint("scr_kc_scorelimit");
				...
				b0, b1, b2, b3, b4, b5, b6, b7, b8, b9 )
{
	...
	precacheString(&"Recover ^3Dogtags ^7from fallen ^2players ^7and ^1enemies^7.");
	...
	if(isPlayer(attacker))
	{
		...
		else
		{
			...
			else
			{
				...
				attacker thread dropTag(self.origin, self);
...
dropTag(org, vic)
{
	...
	trace=bullettrace(org, org-(0,0,1000), false, undefined);
	checkorg=trace["position"];

	self.tag=spawn("script_model", checkorg+(0,0,30));
	self.tag setModel("xmodel/legbag_objective");
	self.tag rotatePitch(-90,0.05);
	self.tag thread moveUpDownLoop();

	while(isDefined(self.tag))
	{
		player=getentarray("player", "classname");
		for(i=0;i<player.size;i++)
		{
			if(distanceSquared(checkorg, player[i].origin) < 50*50 && isAlive(player[i]))
			{
				self.tag delete();

				player[i].score++;
				if(player[i].pers["team"] == self.pers["team"])
				{
					player[i] iprintlnbold("^3Kill Confirmed");
				...
				{
					if(player[i]!=vic)
						player[i] iprintlnbold("^1Denied Kill");
					else
					{
						player[i].score+=2;
						player[i] iprintlnbold("^3Recovered Your Tag");
					}
					return;
				}
			}
...
spawnPlayer()
{
	...
	kcObjective = "Recover ^3Dogtags ^7from fallen Players and Enemies^7.";
	if(self.pers["team"] == "allies")
		self setClientCvar("cg_objectiveText", kcObjective);
	else if(self.pers["team"] == "axis")
		self setClientCvar("cg_objectiveText", kcObjective);
...
updateScriptCvars()
{
	for(;;)
	{
		timelimit = getcvarfloat("scr_kc_timelimit");
		...
		scorelimit = getcvarint("scr_kc_scorelimit");
